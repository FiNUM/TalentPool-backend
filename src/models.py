...
class TempTP(db.Model):
    ...
    professionalExpTable = db.relationship('ProfessionalExperience', backref='temptp', lazy=True)
    applicationsTable = db.relationship('Applications', backref='temptp', lazy=True)
    leadershipTable=db.relationship('Leading_Position', backref='temptp', lazy=True)
